      ...
      <div class="p-4 sm:px-6">
        ...
        <div v-if="!advanced">
          ...
          <div class="mb-4 border-b pb-4">
            ...
            <div class="flex justify-between mt-3">
              <div class="flex flex-col gap-2">
                <Select v-model="generalAccessLevel" :options="levelOptions" @update:model-value="
              ...
              <Select v-if="generalAccessLevel !== 'restricted'" v-model="generalPerms" :options="accessOptions"
                @update:model-value="
          ...
          <div class="flex gap-3">
            <div class="flex-grow">
              <Combobox v-slot="{ open }" v-model="sharedUsers" multiple>
                <div class="flex flex-col items-start justify-center rounded-md bg-surface-gray-2">
                  <div class="flex flex-wrap justify-between p-1 w-full">
                    <div class="w-[75%] flex flex-wrap">
                      <Button v-for="(user, idx) in sharedUsers" :key="user.name" :label="user.email" variant="outline"
                ...
                <div class="absolute z-[4] rounded-lg bg-surface-modal text-base shadow-2xl">
                  <ComboboxOptions v-if="open && query.length" class="max-h-[15rem] overflow-y-auto px-1.5 py-1.5">
                    <ComboboxOption v-if="!filteredUsers.length" v-slot="{ selected, active }" as="template"
                      :value="baseOption">
                      <li class="flex items-center justify-between rounded px-2.5 py-1.5 text-base text-ink-gray-7"
                        :class="{
                    ...
                    <template v-for="person in filteredUsers" :key="person.email">
                      <ComboboxOption v-slot="{ selected, active }" as="template" :value="person"
            ...
            <Select v-model="shareAccess" class="flex items-start min-w-36" :options="advancedTweak
