...
@frappe.whitelist()
def get_global_trash():
    """
    Lấy tất cả file đã xóa (trash) của người dùng hiện tại trên tất cả các team
    """
    trash_items = frappe.db.sql("""
        SELECT dt.entity as name, df.title, df.is_group, df.is_link, df.path, df.modified, 
               df.creation, df.file_size, df.mime_type, df.color, df.document, 
               df.owner, df.parent_entity, df.is_private, df.team,
               CASE 
                   WHEN df.is_group = 1 THEN 'Folder'
                   WHEN df.mime_type IS NOT NULL THEN df.mime_type
                   ELSE 'Unknown'
               END as file_type,
               1 as `read`, 1 as `comment`, 1 as `share`, 1 as `write`,
               dt.trashed_on, dt.original_path,
               CASE WHEN fav.name IS NOT NULL THEN fav.name ELSE NULL END as is_favourite
        FROM `tabDrive Trash` dt
        INNER JOIN `tabDrive File` df ON dt.entity = df.name
        LEFT JOIN `tabDrive Favourite` fav ON fav.entity = df.name AND fav.user = %s
        WHERE dt.user = %s AND df.is_active = 0 AND df.team IN (
            SELECT parent FROM `tabDrive Team Member` 
            WHERE user = %s AND parenttype = 'Drive Team'
        )
        ORDER BY dt.trashed_on DESC
    """, (frappe.session.user, frappe.session.user, frappe.session.user), as_dict=True)
    
    return trash_items


@frappe.whitelist()
def get_global_favorites():
    """
    Lấy tất cả file yêu thích (favourite) của người dùng hiện tại trên tất cả các team
    """
    favorites = frappe.db.sql("""
        SELECT df.name, df.title, df.is_group, df.is_link, df.path, df.modified, 
               df.creation, df.file_size, df.mime_type, df.color, df.document, 
               df.owner, df.parent_entity, df.is_private, df.team,
               CASE 
                   WHEN df.is_group = 1 THEN 'Folder'
               ...
               END as file_type,
               1 as `read`, 1 as `comment`, 1 as `share`, 1 as `write`,
               dv.name as is_favourite
        FROM `tabDrive Favourite` dv
        INNER JOIN `tabDrive File` df ON dv.entity = df.name
        WHERE dv.user = %s AND df.is_active = 1 AND df.team IN (
            SELECT parent FROM `tabDrive Team Member` 
            WHERE user = %s AND parenttype = 'Drive Team'
        )
        ORDER BY dv.creation DESC
...
@frappe.whitelist() 
def get_global_shared():
    """
    Lấy tất cả file được chia sẻ với người dùng hiện tại trên tất cả các team
    """
    shared_files = frappe.db.sql("""
        SELECT DISTINCT df.name, df.title, df.is_group, df.is_link, df.path, df.modified, 
               df.creation, df.file_size, df.mime_type, df.color, df.document, 
               df.owner, df.parent_entity, df.is_private, df.team,
               CASE 
                   WHEN df.is_group = 1 THEN 'Folder'
                   ...
                   ELSE 'Unknown'
               END as file_type,
               dp.`read`, dp.`write`, dp.`share`, dp.`comment`,
               dp.owner as sharer,
               CASE WHEN fav.name IS NOT NULL THEN fav.name ELSE NULL END as is_favourite
        FROM `tabDrive Permission` dp
        INNER JOIN `tabDrive File` df ON dp.entity = df.name
        LEFT JOIN `tabDrive Favourite` fav ON fav.entity = df.name AND fav.user = %s
        WHERE dp.user = %s AND df.is_active = 1 AND df.owner != %s AND df.team IN (
            SELECT parent FROM `tabDrive Team Member` 
            WHERE user = %s AND parenttype = 'Drive Team'
        )
        ORDER BY dp.creation DESC
    """, (frappe.session.user, frappe.session.user, frappe.session.user, frappe.session.user), as_dict=True)
    
    return shared_files


@frappe.whitelist()
def restore_from_global_trash(entity_names):
    """
    Khôi phục các file từ global trash theo danh sách entity name
    """
    if isinstance(entity_names, str):
        entity_names = json.loads(entity_names)
    if not isinstance(entity_names, list):
        frappe.throw(f"Dữ liệu đầu vào phải là list nhưng nhận được {type(entity_names)}", ValueError)
    
    for entity_name in entity_names:
        # Lấy team của file này
        team = frappe.db.get_value("Drive File", entity_name, "team")
        if team:
            # Gọi hàm remove_or_restore để khôi phục
            remove_or_restore([entity_name], team)


@frappe.whitelist()
def empty_global_trash():
    """
    Xóa vĩnh viễn tất cả file trong global trash của người dùng hiện tại
    """
    # Chỉ nhận từ nhóm mà người dùng là thành viên
    trash_items = frappe.db.sql("""
        SELECT dt.entity
        FROM `tabDrive Trash` dt
        INNER JOIN `tabDrive File` df ON dt.entity = df.name
        WHERE dt.user = %s AND df.team IN (
            SELECT parent FROM `tabDrive Team Member` 
            WHERE user = %s AND parenttype = 'Drive Team'
        )
    """, (frappe.session.user, frappe.session.user), as_dict=True)
    
    entity_names = [item.entity for item in trash_items]
    if entity_names:
        # Xóa tệp vĩnh viễn
        for entity in entity_names:
            try:
                ...
                frappe.logger().error(f"Error deleting entity {entity}: {e}")
                continue
        
        # Xóa bản ghi khỏi bảng Drive Trash (chỉ của teams user tham gia)
        frappe.db.sql("""
            DELETE dt FROM `tabDrive Trash` dt
            INNER JOIN `tabDrive File` df ON dt.entity = df.name
            WHERE dt.user = %s AND df.team IN (
                SELECT parent FROM `tabDrive Team Member` 
                WHERE user = %s AND parenttype = 'Drive Team'
...
def get_global_home():
    """
    Lấy dữ liệu tổng hợp cho home page của người dùng hiện tại
    """
    home_files = frappe.db.sql("""
        SELECT df.name, df.title, df.is_group, df.is_link, df.path, df.modified, 
               df.creation, df.file_size, df.mime_type, df.color, df.document, 
               df.owner, df.parent_entity, df.is_private, df.team,
               CASE 
               ...
               1 as `read`, 1 as `comment`, 1 as `share`, 1 as `write`,
               CASE WHEN fav.name IS NOT NULL THEN fav.name ELSE NULL END as is_favourite,
               dt.title as team_name
        FROM `tabDrive File` df
        INNER JOIN `tabDrive Team` dt ON df.team = dt.name
        LEFT JOIN `tabDrive Favourite` fav ON fav.entity = df.name AND fav.user = %s
        WHERE df.is_active = 1 AND df.is_private = 1 AND df.team IN (
            SELECT parent FROM `tabDrive Team Member` 
            WHERE user = %s AND parenttype = 'Drive Team'
        )
